/*
Single-file Node.js chat app (Express + Socket.io) with SMS OTP login.
Usage:
1) Install dependencies:
   npm init -y
   npm install express socket.io twilio body-parser
2) Set environment variables:
   TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER
   (or leave unset to use console-only OTP)
3) Run: node wa-like-single-file-nodejs-server.js
4) Open http://localhost:3000

NOTE: This is a demo. For production: use HTTPS, secure session storage, DB, rate-limiting, input validation.
*/
...
if (TWILIO_SID && TWILIO_TOKEN) {
  try {
    ...
    console.warn('Twilio module not installed or failed. SMS will not be sent.');
  }
} else {
  console.log('TWILIO env not set — SMS OTP will be printed to console only.');
}

const DB_FILE = './db.json';
let DB = { users: {}, otps: {} }; // users keyed by phone: { phone, name, contacts: [phone], messages: {otherPhone: [{from, text, time}]} }

// load DB
try {
  if (fs.existsSync(DB_FILE)) {
    DB = JSON.parse(fs.readFileSync(DB_FILE, 'utf8')) || DB;
  }
} catch (e) {
